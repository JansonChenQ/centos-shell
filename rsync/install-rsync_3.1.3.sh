...
# 增加远程sudo执行，依赖bigdata用户，用于sudo远程同步时权限提升
if [[ `grep -c "^bigdata" /etc/sudoers` = 0 ]]; then
    # 增加sudo权限用于rsync
    echo "bigdata    ALL=NOPASSWD:/data/service/rsync/bin/rsync" >> /etc/sudoers
else
    echo "bigdata用户的sudo执行rsync权限已存在"
fi